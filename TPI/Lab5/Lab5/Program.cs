...
using (IRepository repository = Repository.Create("Celebrities"))
{
    ...
    {
            var celebr = context.GetArgument<Celebrity>(0);
            if (celebr == null)
            ...
            if (celebr.Surname == null || celebr.Surname.Length < 2)
    ...
    .AddEndpointFilter(async (context, next) =>
    {
            ...
            {
                if(celebr.Surname==celebrity.Surname)
                {
    ...
    .AddEndpointFilter(async (context, next) =>
    {
        ...
        if (celebr == null)
            return Results.Problem(detail: "Null error", statusCode: 500);

        string fileName = Path.GetFileName(celebr.PhotoPath);
